...
<head>
    ...
    <title>Customer Due Report Export</title>
    <style>
...
<body>
    <div class="header">
        <h1>Customer Due Report Export</h1>
        ...
        <table>
            <thead>
                <tr>
                    @foreach($columnsToShow as $column)
                        <th class="@if(in_array($column, ['grand_total', 'returned_amount', 'paid', 'due'])) text-right @endif">
            ...
            <tbody>
                @foreach($rows as $row)
                    <tr>
                        @foreach($columnsToShow as $column)
                            <td class="@if(in_array($column, ['grand_total', 'returned_amount', 'paid', 'due'])) text-right @endif">
                                {{ $row[$column] ?? '—' }}
                            </td>
                        @endforeach
                    </tr>
    ...
    @else
        <p class="text-center">No due sales to export.</p>
    @endif
