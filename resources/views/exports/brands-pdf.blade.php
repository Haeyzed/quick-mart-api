...
<body>
    <div class="header">
        ...
        @if(count($brands) > 0)
            <div class="date">Total Records: {{ count($brands) }}</div>
        @endif
    </div>
                            ...
                            <td class="max-width-200">
                                @switch($column)
                                    ...
                                    @case('short_description')
                                        {{ Str::limit($brand->short_description ?? '-', 50) }}
                                        @break
                                    @case('page_title')
                                        {{ $brand->page_title ?? '-' }}
                                        @break
                                    @case('image_url')
                                        {{ Str::limit($brand->image_url ?? '-', 50) }}
                                        @break
                                    @case('is_active')
                                        <span class="badge {{ $brand->is_active ? 'badge-yes' : 'badge-no' }}">
    ...
    @else
        <p class="text-center">No brands to export.</p>
    @endif
