...
<body>
    <div class="header">
        ...
        @if(count($audits) > 0)
            <div class="date">Total Records: {{ count($audits) }}</div>
        @endif
    </div>
                    ...
                    <tr>
                        @foreach($columnsToShow as $column)
                            <td class="max-width-200">
                                @switch($column)
                                    @case('id') {{ $audit->id }} @break
                                    @case('event') {{ $audit->event }} @break
                                    @case('auditable_type') {{ class_basename($audit->auditable_type) }} @break
                                    @case('auditable_id') {{ $audit->auditable_id }} @break
                                    @case('user_name') {{ $audit->user?->name ?? '-' }} @break
                                    @case('ip_address') {{ $audit->ip_address ?? '-' }} @break
                                    @case('created_at') {{ $audit->created_at?->format('Y-m-d H:i:s') ?? '-' }} @break
                                    @default -
                                @endswitch
                            </td>
                        @endforeach
    ...
    @else
        <p class="text-center">No audits to export.</p>
    @endif
