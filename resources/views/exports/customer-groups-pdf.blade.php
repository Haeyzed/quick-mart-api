...
<head>
    ...
    <title>Customer Groups Export</title>
    <style>
...
<body>
    <div class="header">
        <h1>Customer Groups Export</h1>
        <div class="date">Generated on: {{ date('Y-m-d H:i:s') }}</div>
        @if(count($customerGroups) > 0)
            <div class="date">Total Records: {{ count($customerGroups) }}</div>
        @endif
    </div>
                ...
                @foreach($customerGroups as $customerGroup)
                    <tr>
                        @foreach($columnsToShow as $column)
                            <td>
                                @switch($column)
                                    @case('id') {{ $customerGroup->id }} @break
                                    @case('name') {{ $customerGroup->name ?? '-' }} @break
                                    @case('percentage') {{ $customerGroup->percentage ?? '-' }} @break
                                    @case('is_active')
                                        <span class="badge {{ $customerGroup->is_active ? 'badge-yes' : 'badge-no' }}">{{ $customerGroup->is_active ? 'Yes' : 'No' }}</span>
                                        @break
                                    @case('created_at') {{ $customerGroup->created_at?->format('Y-m-d H:i:s') ?? '-' }} @break
                                    @case('updated_at') {{ $customerGroup->updated_at?->format('Y-m-d H:i:s') ?? '-' }} @break
                                    @default -
                                @endswitch
                            </td>
                        @endforeach
    ...
    @else
        <p class="text-center">No customer groups to export.</p>
    @endif
