...
<body>
    <div class="header">
        ...
        @if(count($units) > 0)
            <div class="date">Total Records: {{ count($units) }}</div>
        @endif
    </div>
        ...
        <table>
            ...
            <tbody>
                @foreach($units as $unit)
                    <tr>
                        @foreach($columnsToShow as $column)
                            <td class="max-width-200 {{ in_array($column, ['code', 'operator']) ? 'font-mono' : '' }}">
                                @switch($column)
                                    @case('id')
                                        {{ $unit->id }}
                                        @break
                                    ...
                                    @case('name')
                                        ...
                                        @break
                                    @case('base_unit_name')
                                        {{ $unit->baseUnitRelation?->name ?? '-' }}
                                        @break
                                    @case('operator')
                                        {{ $unit->operator ?? '-' }}
                                        @break
                                    @case('operation_value')
                                        {{ $unit->operation_value !== null ? $unit->operation_value : '-' }}
                                        @break
                                    @case('is_active')
                                        <span class="badge {{ $unit->is_active ? 'badge-yes' : 'badge-no' }}">
    ...
    @else
        <p class="text-center">No units to export.</p>
    @endif
